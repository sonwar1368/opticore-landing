'use client';   // ← این خط رو اول فایل اضافه کن
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 md:px-8 relative overflow-hidden bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-8 md:gap-16 items-center relative z-10">
        <div className="opacity-0 -translate-x-12 animate-fade-in-left">
          ...
          <button className="mt-8 md:mt-12 px-8 md:px-12 py-4 md:py-6 bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-base md:text-xl font-bold rounded-xl md:rounded-2xl shadow-2xl hover:scale-105 transition-all duration-300">
            Get Early Access →
          </button>
        </div>

        <div className="opacity-0 scale-90 animate-fade-in-right">
    ...
    steps:
      ...
      <style jsx global>{`
        ...
        .animate-fade-in-right {
          animation: fade-in-right 1.2s ease-out 0.4s forwards;
        }
        @media (max-width: 768px) {
          .grid { grid-template-columns: 1fr !important; }
          h1, p, button { text-align: center; }
          button { width: 100%; max-width: 320px; margin: 2rem auto 0; display: block; }
        }
      `}</style>
    </main>
  );
